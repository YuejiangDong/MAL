...
class Trainer:
                ...
                for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                    ...
                    if not self.opt.disable_automasking:
                        outputs[("color_identity", frame_id, scale, deq_iter)] = \
                            inputs[("color", frame_id, source_scale)]
        print("debug in gen ********")
        print(outputs[("cam_T_cam", 0, -1, 1)])
        print(outputs[("color", -1, 0, 1)].min(),outputs[("color", -1, 0, 1)].max(), outputs[("color", -1, 0, 1)].mean())
        print("**************")
    ...
    def pose_update_generate_images_pred(self, inputs, outputs):
        ...
        outputs[("color", -1, 0, 0, 1)] = F.grid_sample(
            ...
            pix_coords,
            padding_mode="border", align_corners=False)
        print("in pose update ")
        print(T)
        print(outputs[("color", -1, 0, 0, 1)].min(), outputs[("color", -1, 0, 0, 1)].max(), outputs[("color", -1, 0, 0, 1)].mean())
        exit(0)
