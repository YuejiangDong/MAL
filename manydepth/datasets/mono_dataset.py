...
class MonoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if type(self).__name__ in ["CityscapesPreprocessedDataset", "CityscapesEvalDataset"]:
            ...
            if 1 in self.frame_idxs:
                get_next = True
            inputs.update(self.get_colors(folder, frame_index, side, do_flip, get_next))
        else:
        ...
        # load optical flow
        if self.opt_flow:
            opt_flow_np = self.get_opt_flow(folder, frame_index, side, do_flip)
            opt_flow_ori = torch.from_numpy(opt_flow_np.astype(np.float32))
            for i in range(self.num_scales):
                inputs[("optflow", i)] = self.resize_tensor[i](opt_flow_ori)
        return inputs
    ...
    def get_opt_flow(self, folder, frame_index, side, do_flip):
        raise NotImplementedError
