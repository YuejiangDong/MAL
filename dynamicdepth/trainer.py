...
# from dynamicdepth.dyn_utils import image_synthesis
# from dynamicdepth.loss_utils import compute_mono_losses, compute_main_losses, LossBalancing, compute_mono_losses_v2
...
class Trainer:
    ...
    def train(self):
        ...
        # if self.opt.load_weights_folder is not None:
        #     metrics, mono_metrics = self.val()
        #     print(metrics, mono_metrics)
    ...
    def run_epoch(self):
        ...
        for batch_idx, inputs in iterator:
            ...
            # early val check:
            mono_flag = self.train_teacher_and_pose # and self.opt.dataset == "cityscapes_preprocessed" 
            
            if self.step == 250:
                self.set_eval()
                if self.freeze_tp:
                    metrics,_ = self.val(mono_eval=False) # if not self.opt.ddad else self.val_ddad()
                    print(f"early validate at step {self.step} ", metrics)
                else:
                    metrics, metrics_mono, _ = self.val() # if not self.opt.ddad else self.val_ddad()
            ...
            if self.step != 0 and self.step % self.opt.validate_every == 0 and self.step > self.opt.validate_from:
                ...
                self.set_eval()
                if self.freeze_tp:
                    metrics, doj_metrics = self.val(mono_eval=False) # if not self.opt.ddad else self.val_ddad()
                    print("self.step ", self.step, " validate ", metrics)
                else:
                    metrics, metrics_mono, doj_metrics = self.val() # if not self.opt.ddad else self.val_ddad()
                    print("self.step ", self.step, " validate ", metrics)
                ...
                wandb.log({f"doj/{k}": v for k, v in zip(self.depth_metric_names, doj_metrics)}, step=self.step)
                # print(f"step {self.step} min depth {self.depth_bin_tracker.min_depth}, max depth {self.depth_bin_tracker.max_depth}")
                
                save_folder = os.path.join("../../share/dyj/ckpt", f"{self.opt.name}_s{self.step}")
    ...
    def process_batch(self, inputs, index_iter, is_train=False):
        ...
        if is_train and not self.opt.export:
            ...
            if self.opt.distil:
                ...
                losses, self.w_list, loss_list = compute_main_losses(self.ssim, inputs, outputs, mono_reproj, ensemble_reproj, self.opt, self.w_list, self.multi_has_ins)
    ...
    def val(self, mono_eval=True):
        ...
        if mono_eval:
            for i, metric in enumerate(self.depth_metric_names):
                mono_losses[metric] = 0.0
                mono_losses['doj/'+metric] = 0.0
            mono_losses['doj/count'] = 0.0
            mono_losses['dojpxs'] = 0
            mono_losses['allpxs'] = 0
        total_batches = 0
        ...
        with torch.no_grad():
            for batch_idx, inputs in enumerate(tqdm(self.val_loader)):
                ...
                # predict poses for all frames
                pose_pred = self.predict_poses(inputs, None, False or self.opt.export)
                outputs.update(pose_pred)
                mono_outputs = {}
                mono_outputs.update(pose_pred)
                ...
                self.compute_depth_losses(inputs, outputs, losses, mono_losses, batch_idx, mono_eval, accumulate=True)
        ...
        for i, metric in enumerate(self.depth_metric_names):
            ...
            if mono_eval:
                mono_losses[metric] /= total_batches
                mono_losses['doj/'+metric] /= losses['doj/count']
        ...
        if mono_eval:
            ...
            mono_err = []
        for i, metric in enumerate(self.depth_metric_names):  
            errors.append(losses[metric])
            doj_err.append(losses['doj/'+metric]) 
            if mono_eval:
                mono_err.append(mono_losses[metric])
                mono_doj_err.append(mono_losses['doj/'+metric])
            
        # print("doj ", doj_err, mono_doj_err)
        if mono_eval:
            return errors, mono_err, doj_err
        else:
            return errors, doj_err
    ...
    def compute_losses(self, inputs, outputs, is_multi=False):
        ...
        for scale in self.opt.scales:
            ...
            if self.opt.avg_reprojection:
                ...
                return_mono_rpj = reprojection_loss.clone()
    ...
    def compute_depth_losses(self, inputs, outputs, losses, losses_mono, idx, mono, accumulate=False):
        ...
        if mono:
            _, depth_pred_mono = disp_to_depth(outputs[("mono_disp", 0)], self.opt.min_depth, self.opt.max_depth)
        ...
        if mono:
            depth_pred_mono = torch.clamp(F.interpolate(depth_pred_mono, [gt_height, gt_width], mode="bilinear", align_corners=False), 1e-3, 80)
            depth_pred_mono = depth_pred_mono.detach().squeeze()
        ...
        if self.opt.split in ['cityscapes_preprocessed']:
            ...
            if mono:
                depth_pred_mono = depth_pred_mono[256:, 192:1856]
        ...
        if mono:
            depth_pred_mono *= torch.median(gt_depth[mask]) / torch.median(depth_pred_mono[mask])
            depth_pred_mono = torch.clamp(depth_pred_mono, min=1e-3, max=80)
        ...
        if mono:
            depth_errors_mono = compute_depth_errors(gt_depth[mask], depth_pred_mono[mask])
            doj_errors_mono = compute_depth_errors(gt_depth[doj_mask], depth_pred_mono[doj_mask])
        if doj_mask.sum() > 0:
            losses['doj/count'] += 1
        ...
        for i, metric in enumerate(self.depth_metric_names):
            if accumulate:
                ...
                if mono:
                    losses_mono[metric] += np.array(depth_errors_mono[i].cpu())
                
                if doj_mask.sum() > 0:
                    losses['doj/'+metric] += np.array(doj_errors[i].cpu())
                    if mono:
                        losses_mono['doj/'+metric] += np.array(doj_errors_mono[i].cpu())
